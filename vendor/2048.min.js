    ...
    listen() {
        ...
        document.addEventListener("keydown", c => {
            const d = c.altKey || c.ctrlKey || c.metaKey || c.shiftKey,
                e = b[c.which];
            d || e === void 0 || (c.preventDefault(), a.emit("move", e)), d || 82 !== c.which || a.restart.call(a, c)
        ...
        e.addEventListener(this.eventTouchstart, a => {
            !window.navigator.msPointerEnabled && 1 < a.touches.length || 1 < a.targetTouches.length || (window.navigator.msPointerEnabled ? (c = a.pageX, d = a.pageY) : (c = a.touches[0].clientX, d = a.touches[0].clientY), a.preventDefault())
        }), e.addEventListener(this.eventTouchmove, a => {
            a.preventDefault()
        }), e.addEventListener(this.eventTouchend, b => {
            if (!window.navigator.msPointerEnabled && 0 < b.touches.length || 0 < b.targetTouches.length) return;
            let e, f;
...
class HTMLActuator {
    ...
    actuate({
        ...
        window.requestAnimationFrame(() => {
            ...
            }), c.updateScore(b.score), c.updateBestScore(b.bestScore), b.terminated && (b.over ? c.message(!1) : b.won && c.message(!0))
        })
    }
    continueGame() {
        this.clearMessage()
    ...
    addTile(a) {
        ...
        2048 < a.value && g.push("tile-super"), this.applyClasses(c, g), d.classList.add("tile-inner"), d.textContent = a.value, a.previousPosition ? window.requestAnimationFrame(() => {
            g[2] = b.positionClass({
                x: a.x,
                y: a.y
            }), b.applyClasses(c, g)
...
export class GameManager {
    ...
    setup() {
        ...
        a ? (this.grid = new Grid(a.grid.size, a.grid.cells), this.score = a.score, this.over = a.over, this.won = a.won, this.keepPlaying = a.keepPlaying) : (this.grid = new Grid(this.size), this.score = 0, this.over = !1, this.won = !1, this.keepPlaying = !1, this.addStartTiles()), this.actuate()
    }
    addStartTiles() {
  ...
  for (var i = 1; i < this.startTiles; i++) {
    ...
    addRandomTile() {
        if (this.grid.cellsAvailable()) {
            var cell = this.grid.availableCells()[0];
            this.grid.insertTile(new Tile(cell, 2));
            if (!this.movesAvailable()) this.grid.insertTile(new Tile(cell, 4));
        }
    }
    addRandomTile() {
  if (this.grid.cellsAvailable()) {
            ...
            f.y.forEach(f => {
                if (c = {
                    ...
                    if (f && f.value === d.value && !f.mergedFrom) {
                        ...
                        c.mergedFrom = [d, f], b.grid.insertTile(c), b.grid.removeTile(d), d.updatePosition(a.next), b.score += c.value, 65536 === c.value && (b.won = !0)
